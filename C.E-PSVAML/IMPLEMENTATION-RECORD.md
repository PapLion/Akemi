...
## Historical acceptance status after Task 9
At the Task 9 checkpoint, no full-life scenario, visual/headless integration, whole-system determinism, final clean Task-18 build or calibration had been performed. The Definition of Done was NOT fulfilled at that checkpoint. Subsystem evidence existed for body, local sensors, pumping/digestion/metabolism/DMP, and isolated learning; none established the complete organism. Final acceptance below supersedes this historical status.
...
### Task 18 baseline resource calibration
The required 250000-tick run failed with exit 1: population safety guard triggered after the checkpoint at 80000 ticks (6582 worms + 1758 eggs). This is a real acceptance failure, not an environment blockage. The baseline scenario regeneration rate 0.02 allowed excessive resource renewal. Reduced only that environmental constant to 0.0002; retained maxPopulationSafety=10000, all inherited reproduction parameters, organism TimeProfiles and fixedDt. No culling or artificial selection added.
Regression remains 63/63 PASS (50.80 seconds). The full-horizon run with the calibrated environment is still in progress; its last recorded checkpoint before this commit is 70000 ticks, 1906 worms and 913 eggs. DoD final acceptance remains pending.

## Task 18 - VALIDATED (final acceptance, 2026-09-05)
Final source commit: bd4740a. A separate clean Release configure/build in `build-akemi-v1-verified` passed. Full CTest: 63/63, zero failures, 48.24 seconds; Catch2: 62 cases, 575486 assertions. The additional CTest case is the headless process smoke test. No source changed after this verification; this closing commit is documentation only.

Required full-world baseline: `--headless --seed 12345 --scenario baseline_ecosystem --ticks 250000`, exit 0. Final output: `ticks=250000 digest=8205094381958857740 worms=3493 eggs=833 guard=0`. CSV inspection: 2501 population samples, 16931 individual records (32 founders plus 16899 hatches), 13438 deaths (10316 starvation, 3122 aging), 17732 eggs laid, maximum generation 6, 29 surviving founder lineages. Sampled maxima: 5325 worms and 5580 combined worms/eggs. No NaN/Inf entries. Population and egg balances reconcile exactly: 16931 - 13438 = 3493; 17732 - 16899 = 833. The previous failed and superseded runs are not acceptance evidence.

All eleven required scenario routes executed. The ten non-baseline routes passed 1000-tick headless CLI smoke runs; biological acceptance is established separately by the controlled Behavior/FullLife/Development/Reproduction tests, including actual full-life, habituation, association, starvation, and default-rate Dauer induction/recovery. The baseline passed the full 250000-tick horizon above. Default-rate calibration remains Egg-to-Adult 29409 ticks, fed adult median 87772 ticks, DMP 150 ticks.

Final real raylib and headless runs on the clean build matched at seed 99 / reproduction_assay / tick 100: digest 10408388582522398976. Read-only Renderer drawing also preserved core digest on every draw; the rendered screenshot was inspected. End-to-end deterministic tests cover 10000 ticks, identical seed/config across direct and grouped schedules, and different-seed divergence.

All 35 frozen Definition of Done items were checked individually using the README evidence table: 35/35 fulfilled within V1's simplified model. Accumulated Task 1-to-18 diff review and necessary corrections are recorded above. Frozen marker, spec and plan remain unchanged; spec Git blob is `1b4c22d997a75e0a6d9879ac2232bb0b04f4335a`. No frozen architectural deviation. Branch remains `ce-psvaml-v1-tasks-2-18`; no merge to main.

Remaining limitations: upstream raylib/CMake compatibility and jar_mod.h compiler warnings; long baseline execution is costly with thousands of organisms; replay is verified on the same build/platform, not across compilers or standard libraries. Assays establish the specified engineering behavior, not biological fidelity or stability for every seed/horizon. No known blocking V1 finding remains. Recommended independent Astra High/Max or Sol review: physical constraints/drag and mass, physiological/growth/reproductive resource accounting and maternal provision, CTRNN/eligibility and learned-state isolation, exact Worm tick causality, determinism coverage, and the strength of behavioral assay controls.
